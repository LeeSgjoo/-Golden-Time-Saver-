...
/*
작업 시 주의사항
/api/hospital/me 구현: 세션에 저장된 "user" 정보를 확인하여, 해당 사용자가 속한 병원 정보를 HospitalVO 형태로 반환하는 API가 필요합니다.

calStatus 정책:
0: 센터에서 매칭 요청을 보낸 상태 (병원 대시보드 요청 리스트에 뜸)
1: 병원에서 수락한 상태 (병원 대시보드 수용 환자에 뜸)
2: 진료가 끝나거나 퇴원하여 처리가 완료된 상태

데이터 정합성
- HospitalVO의 필드명(hospName, totalSeat, availSeat 등)이 리액트의 hospital.availSeat 등과 대소문자까지 일치해야 합니다.
- PatientVO의 필드명(patientId, name, ktas, symptoms)도 동일하게 확인해 주세요.
 */
